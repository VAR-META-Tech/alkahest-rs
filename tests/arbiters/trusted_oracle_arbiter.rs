...
fn test_trusted_oracle_arbiter_api_usage_examples() {
    ...
    assert_eq!(decoded_instance.oracle, oracle_demand.oracle);
    assert_eq!(decoded_instance.data, oracle_demand.data);

    // Both methods should produce identical results
    assert_eq!(encoded, encoded_instance);
    assert_eq!(decoded.oracle, decoded_instance.oracle);
    assert_eq!(decoded.data, decoded_instance.data);
}
